...
data_ad = ad.read_h5ad('input/rp_matrix_chip_100kb_ad.h5ad')
samples = pd.read_csv('%s/sample_names_chip.csv' % args.output,header=None)[0].values
try:
...
model.load_weights('%s/weight_chip.h5' % args.output)
...
save_hist(gene_vec,bins,'chip_rp_chip_pred')
...
auc_matrix_ad.write_h5ad('%s/auc_rp_chip_ad.h5ad' % args.output)
