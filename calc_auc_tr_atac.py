...
sample_dhs = sparse.load_npz('%s/atac_dhs.npz' % args.output)
sample_dhs = sample_dhs.toarray()

tr_dhs_ad = ad.read_h5ad('input/tr_dhs_ad.h5ad')

significance_index = pd.read_csv('%s/significance_index_atac.csv' % args.output,header=None)[0].values

tr_dhs_ad = tr_dhs_ad[:,significance_index]
...
with open(f'{args.output}/tr_top_atac.json','w') as file:
...
auc_matrix_ad.write_h5ad('%s/auc_tr_atac_ad.h5ad' % args.output)
